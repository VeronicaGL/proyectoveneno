class Game {
  constructor(canvasId) {
    ...
    this.limitFly = Math.floor(Math.random() * 100);
    this.limitProp = Math.floor(Math.random() * 100);
    ...
    if (!this.intervalId) {
      this.intervalId = setInterval(() => {
        ...
        if (this.tickEnemy > this.limitEnemy) {
          this.addEnemy("PUTERO");
          this.tickEnemy = 0;
          this.limitEnemy = Math.floor(Math.random() * 1000);
        }

        if (this.tickFlies > this.limitFly) {
          this.addFly("PALOMAGREY");
          this.tickFlies = 0;
          this.limitFly = Math.floor(Math.random() * 500);
        }
        if (this.tickProps > this.limitProp) {
          this.addFly("PALOMABLACK");
          this.tickProps = 0;
          this.limitProp = Math.floor(Math.random() * 500);
        }
  ...
  checkCollisionPlayer() {
    this.enemies.forEach((enemy) => {
      const colx =
        this.player.x + this.player.width - 40 >= enemy.x &&
        this.player.x < enemy.x + enemy.width;
      const coly =
        this.player.y + this.player.height - 50 >= enemy.y &&
  ...
  restart() {
    ...
    this.props = [];
    this.tickEnemy = 0;
    this.limitEnemy = Math.floor(Math.random() * 150);
    this.start();
  }
